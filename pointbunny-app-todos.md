...
- [x] ~~**Staff stuck on "Staff account setup incomplete" when invited by 2 businesses / removed and re-invited**~~ ✅ fixed 2026-06-13. **Single-membership rule adopted:** a person belongs to one business at a time and can only be invited elsewhere after being removed (first business that invited them wins). Root cause was the invite-claim using `.maybeSingle()` on an email-only lookup - it errors when 2+ pending rows match the email, so nobody got claimed. Fixes shipped:
  ...
  - **Still open (separate, lower priority):** a *confirmed* account (someone who fully joined a business, then was removed) cannot receive a fresh invite email from a new business, because `inviteUserByEmail` 409s on confirmed accounts and the claim only runs on the invite-link path. The single-membership block now makes this rarer, but joining a new business after removal still needs a link mechanism (e.g. server-generated magic/invite link via Resend, or a consent-claim on normal login).
