...
- [ ] **Cross-business invite of an existing account is impossible (and the error message lies)** - found 2026-06-10 while re-testing the staff SELECT policy change (NOT caused by it - pre-existing gap). Scenario: staff kicked from Business 1, then invited by Business 2. What happens: `admin.inviteUserByEmail()` cannot send to an email that already has an auth account, so the Edge Function returns 409 "This email already has a Pointbunny account. Ask them to log in and they will appear on your team" - but `inviteStaff` (model.js, fnError branch) deletes the just-inserted pending row on any Edge Function error, so there is nothing to claim. Even if the row survived, claiming only runs on the invite-link path (`isInviteAcceptance: true`), deliberately not on normal login. The staff member who clicks an old invite link then hits "Staff account setup incomplete" in `loadBusinessContext`.
  - **Proposed fix:** on the already-registered 409, keep the pending row (don't delete it) and show the owner an accurate message ("They already have an account - they'll be asked to join your team on their next sign-in"). Then on normal login, if no staff row matches `user_id` but a pending row matches the email, show a consent prompt ("Business X invited you to join their team - accept?") and claim only on acceptance. This preserves the no-silent-enrollment safety property.
  - **Diagnostic SQL** (run with the test staff email to confirm state before building):
    ```sql
    select id, business_id, email, user_id, is_active, invited_at, joined_at
    from public.staff
