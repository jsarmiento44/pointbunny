...
- [ ] **Pair Customer-Facing Display to its own terminal** — currently all CFD windows open for a business receive every cart update broadcast, so if two POS devices are running simultaneously (e.g. two cashier tablets), both CFDs show the wrong cart. Fix: assign each device a stable `terminal_id` (UUID, generated once and stored in `localStorage` as `pointbunny_terminal_id`). When the main app opens the CFD popup, pass `?terminal=<terminal_id>` in the URL. Include `terminal_id` on every `CFD_CART_UPDATE` channel message. The CFD window reads its own `terminal_id` from the URL and ignores messages where `data.terminalId !== myTerminalId`. No schema changes needed — all client-side.
