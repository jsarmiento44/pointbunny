...
- [x] ~~**Security: tighten `staff` SELECT policy**~~ ✅ applied 2026-06-10 — dropped `authenticated can read all staff` (`qual = true`), replaced with `read own staff row by email` (`lower(email) = lower(auth.email())`). Migration logged in `pointbunny-admin-updates.md`. **Re-test still pending:** invite acceptance, timeclock sign-in, and the staff list (as owner and as a staff account).
- [ ] **Security: drop the legacy `authenticated can read all <table>` policies** — `menu_items`, `adjustments`, `discount_codes`, `employees` all have a SELECT policy with `qual = true` (any business can read any other business's data). Now that scoped `staff read <table>` policies exist (2026-06-10 migration), the read-all policies are redundant. After a few days of staff-side testing, drop them one at a time and re-test the POS as a cashier after each drop.
- [x] ~~**Security: add `is_active` to the serve-time UPDATE policy on `sales`**~~ ✅ applied 2026-06-13 — `ALTER POLICY "App can record serve time on sales"` now requires `staff.is_active` in its staff subquery, so a deactivated session is refused serve-time/void writes at the DB layer (not just session-level). Owner clause unchanged. Logged in `pointbunny-admin-updates.md` (top entry).
...
- [x] ~~**Staff stuck on "Staff account setup incomplete" when invited by 2 businesses / removed and re-invited**~~ ✅ fixed 2026-06-13. **Single-membership rule adopted:** a person belongs to one business at a time and can only be invited elsewhere after being removed (first business that invited them wins). Root cause was the invite-claim using `.maybeSingle()` on an email-only lookup - it errors when 2+ pending rows match the email, so nobody got claimed. Fixes shipped:
  1. **Claim handles multiple pending rows** (`loadBusinessContext`): fetches all pending rows for the email, claims the earliest (`invited_at ASC`), deletes the rest (auto-clean). Client-side, no deploy needed.
  2. **Block at invite time** (`invite-staff` Edge Function): rejects inviting an email that is already an active member (`user_id` set, `is_active`) of any business. **Requires `supabase functions deploy invite-staff`.**
  3. **`sendInvite` flag**: reactivating a previously-joined member skips the email and re-links by `user_id`.
  4. **Misleading "new password can't be the same" on retry** (`controlAcceptInvite`): the password update ran before the claim, so a failed claim left the user retyping the same password and hitting the recovery-style error. Now the invite-accept path tolerates the "same as old password" error (treats it as already-set) and proceeds to claim.
  - **SQL required** (DELETE policy for auto-clean) + stuck-staff cleanup SQL: see the `[2026-06-13]` entry in `pointbunny-admin-updates.md`.
  - [x] ~~**Existing account can't be re-invited by email**~~ ✅ fixed 2026-06-13 (consent-claim on normal login). When `inviteUserByEmail` reports the account already exists (and the single-membership check cleared them), the Edge Function returns `{ existingAccount: true }` and the app keeps the pending row. On that person's next normal login, `loadBusinessContext` surfaces the pending invite as `PENDING_INVITE_CONSENT`; the controller shows a join-team consent panel (`AuthView.showJoinTeam`); Accept claims the row and discards other pending invites. No more deleting orphaned `auth.users` rows. **Requires `supabase functions deploy invite-staff`.** Optional RLS policy ("invitee can read inviting business") shows the business name in the prompt - see admin-updates `[2026-06-13]`.
...
- [ ] **Custom roles and granular permissions** — owner can view, edit, and create roles from the Settings panel. Each role has a checklist of permissions (e.g. can void sales, can manage menu, can view reports, can invite staff). Currently roles are fixed (Admin/Manager/Cashier) with hardcoded permission checks. Implementation: add a `permissions` JSONB column to the `roles` table (may already exist — check schema), build a permissions editor UI in Settings, and replace hardcoded role-name checks throughout the app with `hasPermission(key)` helper reads. Schema change needed if `permissions` column doesn't exist.
  - ✅ **Foundation built 2026-06-13** — the `hasPermission(key)` engine now exists: `ALL_PERMISSIONS` + `ROLE_PERMISSIONS` map + `hasPermission()` in `model.js`, with the agreed capability→role matrix (11 keys). UI gating is centralized in `_applyPermissionGates()` (controller.js) via `data-perm` (hide), `data-perm-disable` (disable + message), and `data-perm-action` (keep visible, strip click), plus runtime backstops in every panel opener. Hardcoded `role === 'Admin'` checks were replaced with `hasPermission()`. **Remaining for this item:** the `roles.permissions` JSONB column + a Settings editor UI so owners can define custom roles (reuse the same capability keys as defaults — see SYSTEM.md 5.11b). Also fixed the leak where the home dashboard stat cards let cashiers open Reports.
  - [x] ~~**Live role-change propagation**~~ ✅ shipped 2026-06-13 — `watchStaffDeactivation` now also takes an `onRoleChanged` callback; the session stores its `role_id` (`state.currentStaff.roleId`) and when a realtime UPDATE to the user's own staff row carries a different `role_id`, `_handleStaffRoleChanged` shows a toast and does a clean `window.location.reload()` (session kept, so `initApp` re-reads the role + re-gates). Confirmation toast after reload via a `pointbunny_role_changed` sessionStorage flag. Deactivation still wins over a role change. No schema change (realtime publication on `staff` already enabled for the deactivation kick-out).
